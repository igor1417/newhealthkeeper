...
<div class="col-lg-10 col-md-10 col-sm-9 right-col">
    <ul id="three-tabs" class="nav nav-tabs">
        ...
        <li class="active"><a href="<?php echo WEB_URL ;?>articles">Articles</a></li>
    ...
    <div id="three-tabsContent" class="tab-content">
        ...
        <div class="tab-pane fade active in row" id="tab_articles">
            ...
            <?php foreach ($posts as $post):?>
                ...
                <div class="article-content">
                    <div class="col-lg-9 col-md-9 col-sm-8 col-xs-8 col-sxs-12">
                        <h1><a href="<?=WEB_URL;?>/post/<?=$post['id_post']?>"><?= !empty($post['title_post']) ? $post['title_post'] : "&nbsp;"; ?></a></h1>
                        <h6 class="author-time">
                            <a href="<?= $post['link_post'] ?>" target="_blank" class="author-time"><?=$post['username_profile']?></a>
                            <?=' ', !empty($post['date_post']) ? $post['date_post'] : '' ; ?>  
                        </h6>
            ...
            <? if ($pageCount > 1):?>
                <ul class="paginator">
                    <? if ($pageNum > 1): ?>
                        <li><a href="articles?pageNum=<?= $pageNum-1; ?>">Back</a></li>
                    <? endif; ?>
                    <? if ($pageNum<=$pagerVisibleSigns+1): ?>
                        <? for ($i=1; $i<=$pageNum+$pagerVisibleSigns; ++$i): ?>
                            <li <?php if ($i == $pageNum): ?> class="selected" <?php endif; ?>><a href="articles?pageNum=<?= $i; ?>"><?= $i; ?></a></li>
                        <? endfor; ?>
                        <? if ($pageNum+$pagerVisibleSigns == $pageCount): ?>
                        <? elseif ($pageNum+$pagerVisibleSigns+1 == $pageCount): ?>
                            <li><a href="articles?pageNum=<?= $pageCount; ?>"><?= $pageCount; ?></a></li>
                        <? else: ?>
                    ...
                    <? elseif ($pageNum > $pageCount - $pagerVisibleSigns): ?>
                        ...
                        <? elseif ($pageNum-$pagerVisibleSigns-1 == 1): ?>
                            <li><a href="articles?pageNum=1">1</a></li>
                        <? else: ?>
                            <li><a href="articles?pageNum=1">1</a></li>
                            <li class="pag-space">...</li>
                        <? endif; ?>
                    ...
                    <? if ($pageNum < $pageCount): ?>
                        <li><a href="articles?pageNum=<? echo $pageNum + 1; ?>">Next</a></li>
                    <? endif; ?>
                </ul>
            <? endif; ?>
